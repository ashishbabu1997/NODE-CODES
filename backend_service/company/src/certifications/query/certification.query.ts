export default {
    getCompanyCertifications: "SELECT certificate_id AS certificationId, certificate_number AS CertificationNumber, logo, document AS certificationDocument, certification_type AS CertificationType  FROM company_certifications WHERE company_id  = $1 AND status = true ORDER BY 1 DESC",
