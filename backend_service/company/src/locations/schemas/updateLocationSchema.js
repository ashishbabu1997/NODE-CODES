...
exports["default"] = Joi.object().keys({
    streetAddress1: Joi.string().required().error(function (errors) {
    ...
    streetAddress2: Joi.string().required().error(function (errors) {
        errors.forEach(function (err) {
            switch (err.code) {
                case "any.required":
    ...
    city: Joi.string().required().error(function (errors) {
        errors.forEach(function (err) {
            switch (err.code) {
                case "any.required":
    ...
    zipCode: Joi.number().required().error(function (errors) {
        errors.forEach(function (err) {
            console.log(err);
            switch (err.code) {
                case "any.required":
    ...
    stateId: Joi.number().required().error(function (errors) {
        errors.forEach(function (err) {
            console.log(err);
            switch (err.code) {
                case "any.required":
    ...
    locationId: Joi.number().required().error(function (errors) {
        errors.forEach(function (err) {
            switch (err.code) {
                case "any.required":
