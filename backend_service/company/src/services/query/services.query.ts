export default {
    ...
    getCompanyTechnologyAreas: `SELECT t.technology_name as "technologyName",t.technology_area_id as "technologyAreaId" from technology_areas t left join company_technology_areas ct on ct.technology_area_id = t.technology_area_id AND ct.status = true WHERE ct.company_id = $1 AND t.status = true`,
    getSupportingDocument: `SELECT supporting_document as "supportingDocument" from company where company_id = $1 and status = true`,
