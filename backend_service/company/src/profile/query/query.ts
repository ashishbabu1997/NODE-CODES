export default {
    ...
    getCompanyProfile:'SELECT c.company_profile_url as "profileUrl",c.company_name as "name",c.company_domain as "domain",c.company_description as "description",c.company_logo as "logo",c.company_cover_page as "coverPage",c.company_tagline as "tagline",c.company_linkedin_id as "linkedinId",c.company_type as "accountType",c.user_id as "roleId",c.company_size_id "companySizeId",c.company_website as "website",e.account_type,e.user_role_id FROM company  c left join employee e on e.company_id=c.company_id WHERE c.company_id = $1 and e.primary_email=true',
