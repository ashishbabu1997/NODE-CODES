...
export const getPositionFilter = (_body) => {
    return new Promise((resolve, reject) => {
        ...
        {
            const query = 'with filter as (select ARRAY(select distinct company_name from company c left join employee e on c.company_id = e.company_id where e.account_type = 1 and c.status = true and e.status = true order by company_name) as company, (select ARRAY(select distinct position_name from positions where status = true and job_status = 6 order by position_name)) as position, (select ARRAY(select json_build_object(\'employeeId\', employee_id, \'employeeName\', firstname) from employee where account_type = 3 and status = true)) as "allocatedList", (select ARRAY( select json_build_object(\'skillId\', skill_id, \'skillName\', skill_name) from skills where status = true )) as skills, (select ARRAY( select json_build_object(\'jobCategoryId\', job_category_id, \'jobCategoryName\', job_category_name) from job_category where status = true )) as jobCategory ) select * from filter';
