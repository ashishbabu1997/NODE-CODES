  ...
  return new Promise((resolve, reject) => {
    (async () => {
      ...
      try {
        ...
        resolve({ code: 200, message: 'Positions listed successfully', data: { positions: steps, totalCount: counts.rows[0].totalCount, counts } });
      } catch (e) {
      ...
      try {
        ...
        if (_body.publish == true) {
          ...
          if (_body.userRoleId == 1) {
            ...
            await createHirerNotifications({
              positionId,
              companyId: _body.cmpId,
              message: hirerMessage,
              candidateId: null,
  ...
  return new Promise((resolve, reject) => {
    ...
    database().query(CompanyQuery, (error, results) => {
      if (error) {
        ...
        return;
      } else resolve({ code: 200, message: 'Companies listed successfully', data: { companies: results.rows } });
    });
  }).catch((err) => {
