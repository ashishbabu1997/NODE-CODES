...
export const createProviderNotifications = async (_body) => {
  return new Promise((resolve, reject) => {
    const currentTime = Math.floor(Date.now());
    (async () => {
      const client = await database().connect();
      try {
        ...
        const changePositionStatusQuery = {
          name: 'create-notification',
          text: notificationsQuery.createProviderNotification,
          values: [_body.candidateId, _body.message, _body.companyId, _body.notificationType, currentTime, _body.userRoleId, _body.employeeId, _body.image, _body.firstName, _body.lastName],
        };
        await client.query(changePositionStatusQuery);
        await client.query('COMMIT');
        resolve({ code: 200, message: 'Notifiaction added successfully', data: {} });
