...
export const removeCandidateFromPositionEmail = async (_body, client) => {
  ...
  try {
    ...
    await createNotification({
      ...
      message,
      candidateId,
      notificationType: 'candidateChange',
...
export const linkCandidateWithPositionEMail = async (_body, client) => {
  try {
    ...
    let positionName = '';
    let hirerEmail = '';
    let hirerCompanyId = null;
    let recruiterEmail = '';
    let recruiterName = '';
    const candidateCount = _body.count;
    let candidateNames = '';
    let cost = '';
    ...
    candidateList.forEach(async (element) => {
      ...
      cost =
        positionResult.rows[0].isellowRate == false
          ? ''
          : element.ellowRate > 0
          ? `${utils.constValues('currencyType', element.currencyTypeId)} ${element.ellowRate} / ${utils.constValues('billType', element.billingTypeId)}\n`
          : '';
      ...
      if (_body.userRoleId == 1) {
        ...
        if (utils.notNull(name))
          requiredCandidateData.push({ name: 'Name of the Candidate', value: utils.capitalize(res.rows[0].candidate_first_name) + ' ' + utils.capitalize(res.rows[0].candidate_last_name) });
    ...
    const message =
      candidateCount == 1 ? `${candidateCount} candidate has been added for the position ${positionName}` : `${candidateCount} candidates has been added for the position ${positionName}`;
    createHirerNotifications({
      positionId: _body.positionId,
      companyId: hirerCompanyId,
      message: message,
      candidateId: null,
      notificationType: 'candidateList',
      userRoleId: _body.userRoleId,
      employeeId: _body.employeeId,
      image: null,
      firstName: null,
...
export const shareAppliedCandidatesPdfEmails = async (_body, client) => {
  try {
    ...
    const availability = utils.notNull(ready_to_start) ? `${utils.constValues('readyToStart', ready_to_start)}\n` : '';
    cost =
      positionResult.rows[0].isellowRate == false ? '' : ellowRate > 0 ? `${utils.constValues('currencyType', currencyTypeId)} ${ellowRate} / ${utils.constValues('billType', billingTypeId)}\n` : '';
...
export const updateAvailabilityNotificationMails = async (_body, client) => {
  try {
    ...
    const ellowAdmins = await client.query(queryService.getEllowAdmins());
    const message =
      _body.availability == true ? `The freelancer, ${_body.candidateName} is ready to apply for positions` : `The freelancer, ${_body.candidateName} is no longer available for any openings`;
    createNotification({
      positionId: null,
      companyId: _body.companyId,
      message: message,
...
export const ellowCandidateReporterFetchMail = async (_body) => {
  try {
    ...
    const replacements = {
      link:'http://localhost:3000/manager-details?token='+_body.uniqueId
    };
