export default {
    ...
    listCandidates:'SELECT c.candidate_name as "candidateName" ,p.company_name as "companyName",s.position_name as "positionName",c.status as "status",c.candidate_status as "candidateStatus" FROM candidate c INNER JOIN company p ON p.company_id=c.company_id INNER JOIN positions s ON c.position_id=s.position_id WHERE c.position_id=$1 '
