...
// >>>>>>> FUNC. >>>>>>>
// blacklist or revert blacklist candidate    
export const parse = (_body) => {
    return new Promise((resolve, reject) => {
        (async () => {
            const client = await database().connect()
            try {
                let res = await client.query(queryService.getCand(_body));

                let parser = JSON.parse(res.rows[0]['resume_data']);
                let parsedHtml = parser['ResumeParserData']['DetailResume'];
                // let p = new DOMParser().parseFromString('<hi>Hello</hi>','text/html')
                let stringWithTabRemoved = parsedHtml.replace("\\t","");
                let s = stringWithTabRemoved.split('\n'),tempString='',i=0,reqObject={};
                // console.log("parsedHtml : ",s);
                s.forEach(element => {
                    if(element=='')
                    {
                        reqObject[i] = tempString.trim();
                        i++;
                        tempString='';
                    }
                    else
                ...
                resolve({ code: 200, message: "Parsed", data: reqObject });
            } catch (e) {
                console.log(e)
                await client.query('ROLLBACK')
                reject({ code: 400, message: "Failed. Please try again.", data: e.message });
