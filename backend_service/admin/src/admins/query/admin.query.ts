export default {
    ...
    allRegisteredUsersList:'SELECT e.firstname as "firstName", e.lastname as "lastName", e.employee_id as "employeeId", e.email as "email", e.admin_approve_status as "adminApproveStatus", e.telephone_number as "phoneNumber", e.account_type as "accountType", c.company_name as "companyName", c.company_website as "companyWebsite" FROM employee e INNER JOIN company c ON c.company_id = e.company_id WHERE e.admin_approve_status IS NOT NULL AND e.account_type=$1',
    clearanceQuery:'UPDATE employee SET status=$2,admin_approve_status=$3,updated_on=$4 WHERE employee_id=$1',
    approveEmployeeQuery:'UPDATE employee SET status= true, admin_approve_status=1, password=$2, updated_on=$3 WHERE employee_id = $1 RETURNING email',
