export default {
    getSkills: `select s.skill_id as "skillId", s.skill_name as "skillName" from  skills s left join job_category_skills jcs on jcs .skill_id = s.skill_id and jcs.status = true where jcs.job_category_id = $1 and s.status  = true`,
